...
\begin{equation}
\begin{split}
  &\mathbb{P}_{\mathrm{SVC}}(m \mid \bx, z \in \{l, m\}) \, \mathbb{P}_{\mathrm{SVC}}(l \mid \bx) = \\
  &\mathbb{P}_{\mathrm{SVC}}(l \mid \bx, z \in \{l, m\}) \, \mathbb{P}_{\mathrm{SVC}}(m \mid \bx)\;,
\end{split}
\end{equation}

which is just a restatement of Bayes' theorem. Since the probabilities $\mathbb{P}_{\mathrm{SVC}}(l \mid \bx)$ are unknown, we can use the above equation (which must hold for all pairs of classes) to set up an optimization problem to solve for the probabilities $\mathbb{P}_{\mathrm{SVC}}(l \mid \bx)$~\cite{libsvm,wu_2004}.
