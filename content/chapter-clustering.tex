...
\begin{algorithm}[htbp]
  ...
  \begin{algorithmic}[1]
    ...
    \While{not converged}
      ...
      \State Update classifier: $\boldsymbol{\gamma} \gets \argmax_{\boldsymbol{\gamma}} S_{\mathrm{CLS}}(\boldsymbol{\gamma}, \mathcal{D}(\mathbf{z}))$.
...
\smallskip
In order for the sequence to terminate, $N_{\mathrm{update}}$ must be such that at most one point per cluster changes its membership per iteration (for example $N_{\mathrm{update}} = 1$). Furthermore, the set of admissible clusters $\mathcal{A}^{(t)}(\bx_i)$ must contain the current assignment, i.e. $z_i^{(t)} \in \mathcal{A}^{(t)}(\bx_i),\text{ } \forall i \in \mathcal{O} \text{ and } \forall t>0$. If the two above assumptions are satisfied, performing a deterministic membership update cannot decrease the global score, i.e. $S_{\mathrm{global}}(\boldsymbol{\beta}^{(t)}, \mathbf{z}^{(t+1)}) \geq S_{\mathrm{global}}(\boldsymbol{\beta}^{(t)}, \mathbf{z}^{(t)})$. Since the regressors are trained to maximize their local scores (whose sum yields the global score), the re-training step cannot decrease the global score, i.e. $S_{\mathrm{global}}(\boldsymbol{\beta}^{(t+1)}, \mathbf{z}^{(t+1)}) \geq S_{\mathrm{global}}(\boldsymbol{\beta}^{(t)}, \mathbf{z}^{(t+1)})$. Finally, since the classifier only affects $\mathcal{A}^{(t)}(\bx_i)$, which we assumed to always contain $z_i$, the score is non-decreasing. Recalling that $Z$ is finite, the sequence $\mathbf{z}^{(t)}$ must terminate. 
...
\begin{equation}
  ...
  \begin{aligned}
    ...
    \log \p{\data_{l^\prime} \cup \{\mathbf{d}_i\} \mid \boldsymbol{\beta}_{l^\prime}} &= \log \p{\data_{l} \mid \boldsymbol{\beta}_{l^\prime}} + \log \p{\mathbf{d}_i \mid \data_{l}, \boldsymbol{\beta}_{l^\prime}} \;.
  \end{aligned}
  \right.
...
Increasing the dimensionality of the input space leads to a higher fraction of mis-clustered points, especially for small training sets. The percentage improves as the training set grows. \red{The results of our model clustering scheme are more robust than the ones of a \gls{dp}-\gls{gmm} clustering scheme. Indeed, the latter severely underperforms in high dimensions.}
...
In Fig.~\ref{fig:clustering:test_outliers}, the vertical axis reports the score (or \gls{mse}) of the initial, exactly partitioned configuration, whereas the horizontal axis reports the same quantity for the final configuration obtained by running the clustering algorithm. Results show that the global score consistently increases from the initial to the final configuration, confirming that the algorithm works and mis-clustered points are due to data scarcity rather than to algorithmic failure. Furthermore, we see that for $100$ observations, the final \gls{mse} is mostly worse than the initial \gls{mse}, while for $500$ observations, there are no visible differences. Overall this analysis confirms that the global score is not a good proxy for the quality of the clustering when data is scarce, but becomes more reliable as the training set grows, which explains the improvement in mis-clustering with more data.
...
In Fig.~\ref{fig:clustering:tc2:mc}, we show the clustering result in 1D and 2D. The algorithm was initialized with an initial K-means clustering of size $10$ and a temperature of $1.0$. As we can see, in both cases the algorithm manages to converge to an excellent clustering. In both cases, the algorithm was able to identify and isolate the inner region (without creating disconnected clusters like the \gls{dp}-\gls{gmm} algorithm) but split the outer region into multiple clusters. However, all clusters are connected and have a regular shape.
...
In this chapter we have presented a novel domain decomposition approach for surrogate modeling of complex functions. The framework is based on a model-based clustering algorithm that using information from local surrogate models and a classifier, is able to cluster the input space into subdomains where the function is smooth and can be effectively approximated by a local surrogate model. The proposed algorithm is iterative, and we have shown that, under some reasonable assumptions, it is guaranteed to converge in a finite number of steps.
