...
%%%%---- DRAWING & GRAPHS ----------------------%%%%
	\usepackage{import}
	\usepackage{tikz} 	%
	\usetikzlibrary{positioning,shapes,shapes.arrows,arrows.meta}
	\usepackage{pgfplots} 	%
...
%%%%---- INDEX & GLOSSARIES --------------------%%%%
	\usepackage[record, section=section]{glossaries-extra}
		%Newglossaries
...
\newcommand{\likelihood}[1]{\mathcal{L}\mleft( #1\mright)} % likelihood
\newcommand{\prior}[1]{\pi \left( #1\right)} % prior 
\newcommand{\p}[1]{\text{p} \left( #1 \right)} % pdf
\newcommand{\pq}[1]{\text{q} \left( #1 \right)} % un-normalized pdf
\newcommand{\normalpdf}{\mathcal{N}}
